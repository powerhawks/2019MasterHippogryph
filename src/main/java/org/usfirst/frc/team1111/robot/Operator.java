...
public class Operator {
	...
	int TPI = 449;
	//lift pid
	public double pT = .0005, iT = 0.00000, dT = 0; //196:1 Gear ratio new at districts
	// public double pT = .0005, iT = 0.0000045, dT = 0; //196:1 Gear ratio //from owings mills
...
// public double pI = .0001, iI = 0.000000, dI = .00000;
	...
	public void operate() {
		...
		if(joystickDriver.getRawButton(ControllerMap.O_LEFT_BUMPER)) {
			flap = false;
			moveToLevel(Dimensions.PANNEL_LVL1 + 5000);
		} 
	...
	public void intakeReset() {
		if (joystick.getRawButton(8)) {
			...
			SmartDashboard.putString("reset", "zero");
		} else if (joystick.getRawButton(7)) {
			Motors.intakeAngle.setSelectedSensorPosition(1334,0,30);
			System.out.println("RESET NOT 0");
			SmartDashboard.putString("reset", "not zero");
		}
	}
	...
	public void intake() {
		if (joystick.getRawButton(ControllerMap.O_A_BUTTON) || Robot.annoyLiam < 5) {
			...
			Motors.intake.set(ControlMode.PercentOutput, -.5); //used to be 1
		} else if(joystick.getRawButton(ControllerMap.O_X_BUTTON)){
	...
	public double ariBreakSpeed(){
		...
		double result;
		int toBoard = 1344; //may need to change
		ticks = toBoard - ticks;
	...
	public double ariIntakeSpeed() {
		int ticks =  Motors.intakeAngle.getSelectedSensorPosition();
		// System.out.println("Ticks Intake" + ticks);
		// if(ticks > 1344) {ticks = 1344;}
		// int result = ticks / 1344;
		int toBoard = 1344; //may need to change
		ticks = toBoard - ticks;
	...
	public void intakeAngle() {
		...
		// if(isLiftReady() || OVERRIDEANGLE){```````````````````````````
			...
			// variable speed currently set to: 0.45 (used in lower wrist)
			if (!joystick.getRawButton(ControllerMap.O_A_BUTTON)) {
					if (joystick.getRawAxis(ControllerMap.O_LEFT_STICK) > 0) {
						Motors.intakeAngle.set(ControlMode.PercentOutput, joystick.getRawAxis(ControllerMap.O_LEFT_STICK) * speed); //sets wrist angle going down
					} else if (joystick.getRawAxis(ControllerMap.O_LEFT_STICK) < 0) {
						Motors.intakeAngle.set(ControlMode.PercentOutput, joystick.getRawAxis(ControllerMap.O_LEFT_STICK) * .6); //sets wrist angle going up
						//Motors.intakeAngle.set(ControlMode.PercentOutput, joystick.getRawAxis(ControllerMap.O_LEFT_STICK) * ariIntakeSpeed() * .75); //sets wrist angle going up
	...
	public void flappyBird() {
		...
		// 	flap = false;
		// }
		if (joystick.getRawButton(ControllerMap.O_Y_BUTTON) && timer >= 10) {
			controlMode = !controlMode;
			timer = 0;
